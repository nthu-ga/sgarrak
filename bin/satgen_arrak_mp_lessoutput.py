...
def process_tree(itree ,fd=0.0 ,flattening=25.,
    ...
    host = sga.Host(host_mass_history, tree_redshifts, cosmology,
                    fd=fd, flattening=flattening,disk_method=disk_method ,output_zred=output_zred)
    
    # Define result keys once
    result_keys = ['prog_masses', 'prog_mstars', 'status', 'radii']
    ...
    for iprog in range(0,nprogs_this_tree):
        ...
        results['initial_mass'].append(prog.mass_init)
        results['initial_mstar'].append(prog.mstar_init)
        
        results['final_mass'].append(solution['prog_masses'][-1])
        results['final_mstar'].append(solution['prog_mstars'][-1])
        results['final_radius'].append(solution['radii'][-1])
        results['final_status'].append(solution['status'][-1])
        
    results['nprog'] = len(results['final_mass'])
